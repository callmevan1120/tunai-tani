document.addEventListener("DOMContentLoaded", () => {
  ...
  // Dummy Data for Reserve Fund (for demonstration)
  const dummyPetaniReserveData = [
    {
      projectId: "1",
  ...
  const dummyKonsumenReserveData = [
    {
      orderId: "ORD001",
  ...
  const renderContent = (feature) => {
        ...
        htmlContent += `
                      ...
                      <div class="card p-3 mb-3">
                          ...
                          <div id="keranjang-summary" class="d-none">
                              ...
                              <p class="small text-muted mb-2">Termasuk 5% alokasi ke Dana Cadangan.</p>
                              <button class="btn btn-success w-100"><i class="fas fa-money-check-alt me-2"></i>Lanjutkan Pembayaran</button>
                          </div>
                      </div>
                  `;
        break;
      case "dana-cadangan": // New feature: Dana Cadangan
        htmlContent += `
                      <h4 class="mb-3 text-center"><i class="fas fa-shield-alt me-2"></i>Dana Cadangan</h4>
                      <p class="text-center text-muted small">Mitigasi risiko gagal panen untuk keamanan investasi Anda.</p>
                  `;
        if (currentUserRole === "petani") {
          const totalPetaniReserve = dummyPetaniReserveData.reduce(
            (sum, item) => sum + item.reserveAmount,
            0
          );
          htmlContent += `
                          <div class="card p-3 mb-3">
                              <h6 class="mb-2">Informasi Dana Cadangan Proyek Anda</h6>
                              <p class="small text-muted">Dari setiap dukungan pre-order pada proyek Anda, 5% akan dialokasikan ke Dana Cadangan. Dana ini dikelola oleh sistem untuk perlindungan konsumen jika terjadi gagal panen. Dana tidak ditarik oleh petani.</p>
                              <div class="alert alert-info py-2 small">
                                  <i class="fas fa-info-circle me-2"></i>Total Dana Cadangan Terkumpul dari Proyek Anda: <span class="fw-bold">Rp ${totalPetaniReserve.toLocaleString(
                                    "id-ID"
                                  )}</span>
                              </div>
  
                              <h6 class="mt-3 mb-2">Detail Dana Cadangan per Proyek</h6>
                              <div class="table-responsive">
                                  <table class="table table-sm table-bordered small">
                                      <thead>
                                          <tr>
                                              <th>Proyek</th>
                                              <th class="text-end">Dukungan Terkumpul</th>
                                              <th class="text-end">Dana Cadangan</th>
                                          </tr>
                                      </thead>
                                      <tbody>
                                          ${dummyPetaniReserveData
                                            .map(
                                              ...
                                              <tr>
                                                  ...
                                                  <td class="text-end">Rp ${item.totalContribution.toLocaleString(
                                                    "id-ID"
                                                  )}</td>
                                                  <td class="text-end">Rp ${item.reserveAmount.toLocaleString(
                                                    "id-ID"
                                                  )}</td>
                                              </tr>
                                          `
                              ...
                              <p class="small text-muted mb-0">Catatan: Dana cadangan ini adalah jaring pengaman bagi konsumen dan tidak mengurangi modal yang Anda terima.</p>
                          </div>
                          <div class="card p-3 mb-3">
                              <h6 class="mb-2">Distribusi Dana Cadangan (Total)</h6>
                              <canvas id="petaniReserveChart"></canvas>
                          </div>
                      `;
        } else if (currentUserRole === "konsumen") {
          const totalKonsumenContribution = dummyKonsumenReserveData.reduce(
            (sum, item) => sum + item.reserveAllocated,
            0
          );
          const totalKonsumenRefund = dummyKonsumenReserveData.reduce(
            (sum, item) => sum + (item.refundAmount || 0),
            0
          );
          htmlContent += `
                          <div class="card p-3 mb-3">
                              <h6 class="mb-2">Dana Cadangan Anda</h6>
                              <p class="small text-muted">Dari setiap kontribusi pre-order Anda, 5% dari jumlah dukungan dialokasikan ke Dana Cadangan. Dana ini dikelola oleh sistem untuk pengembalian sebagian jika panen gagal total atau sebagian.</p>
                              <div class="alert alert-success py-2 small">
                                  <i class="fas fa-coins me-2"></i>Total Kontribusi Dana Cadangan Anda: <span class="fw-bold">Rp ${totalKonsumenContribution.toLocaleString(
                                    "id-ID"
                                  )}</span>
                              </div>
                              <div class="alert alert-info py-2 small ${
                                totalKonsumenRefund === 0 ? "d-none" : ""
                              }">
                                  <i class="fas fa-hand-holding-usd me-2"></i>Total Pengembalian Dana Cadangan Diterima: <span class="fw-bold text-success">Rp ${totalKonsumenRefund.toLocaleString(
                                    "id-ID"
                                  )}</span>
                              </div>
  
                              <h6 class="mt-3 mb-2">Riwayat Kontribusi & Pengembalian Dana Cadangan</h6>
                              <div class="table-responsive">
                                  <table class="table table-sm table-bordered small">
                                      <thead>
                                          <tr>
                                              <th>Tanggal</th>
                                              <th>Proyek</th>
                                              <th class="text-end">Kontribusi DC</th>
                                          ...
                                          ${dummyKonsumenReserveData
                                            .map(
                                              ...
                                              <tr>
                                                  <td>${item.date}</td>
                                                  <td>${item.projectName}</td>
                                                  <td class="text-end">Rp ${item.reserveAllocated.toLocaleString(
                                                    "id-ID"
                                                  )}</td>
                                                ...
                                                item.refundAmount
                                                  ? `Rp ${item.refundAmount.toLocaleString(
                                                      "id-ID"
                                                    )}`
                                                  : "-"
                                              }</td>
                              ...
                              <p class="text-muted text-center small mt-3 ${
                                dummyKonsumenReserveData.length === 0
                                  ? ""
                                  : "d-none"
                              }">Belum ada riwayat kontribusi dana cadangan.</p>
                          </div>
                          <div class="card p-3 mb-3">
                              <h6 class="mb-2">Komposisi Dana Cadangan Anda</h6>
                              <canvas id="konsumenReserveChart"></canvas>
                          </div>
                      `;
        }
        break;
          ...
          cartItems.forEach((item, index) => {
            ...
            const itemHtml = `
              ...
              item.petani
                                      ...
                                      <p class="small mb-0 text-info">Alokasi Dana Cadangan: <span class="fw-bold">Rp ${item.reserveFund.toLocaleString(
                                        "id-ID"
                                      )}</span></p>
                                  </div>
      ...
      document
        ...
        .forEach((button) => {
          button.addEventListener("click", (e) => {
            ...
            const reserveFundAmount = amount * 0.05; // 5% for reserve fund
            ...
            if (existingItemIndex > -1) {
              ...
              cartItems[existingItemIndex].reserveFund += reserveFundAmount;
            } else {
              cartItems.push({
                id: projectId,
                name: projectName,
                ...
                amount: amount,
                reserveFund: reserveFundAmount, // Store reserve fund for this item
              });
            }
            ...
            alert(
              `${projectName} (Rp ${amount.toLocaleString(
                "id-ID"
              )}) telah ditambahkan ke keranjang pre-order Anda! (Termasuk Rp ${reserveFundAmount.toLocaleString(
                "id-ID"
              )} untuk Dana Cadangan)`
            );
            document.getElementById(inputAmountId).value = "";
    ...
    // Dana Cadangan Charts (New)
    if (feature === "dana-cadangan") {
      if (currentUserRole === "petani") {
        const petaniReserveChartCtx =
          document.getElementById("petaniReserveChart");
        if (petaniReserveChartCtx) {
          new Chart(petaniReserveChartCtx.getContext("2d"), {
            type: "bar",
            data: {
              ...
              datasets: [
                {
                  ...
                  data: dummyPetaniReserveData.map(
                    (item) => item.reserveAmount
                  ),
        ...
        const konsumenReserveChartCtx = document.getElementById(
          "konsumenReserveChart"
        );
        if (konsumenReserveChartCtx) {
          const totalContributions = dummyKonsumenReserveData.reduce(
            (sum, item) => sum + item.reserveAllocated,
            0
          );
          const totalRefunds = dummyKonsumenReserveData.reduce(
            (sum, item) => sum + (item.refundAmount || 0),
            0
          );
          const activeBalance = totalContributions - totalRefunds;

          new Chart(konsumenReserveChartCtx.getContext("2d"), {
            type: "pie",
            data: {
              labels: ["Dana Cadangan Aktif", "Dana Cadangan Dikembalikan"],
              datasets: [
                {
                  data: [activeBalance, totalRefunds],
                  backgroundColor: [
                    "rgba(54, 162, 235, 0.8)",
                    "rgba(255, 99, 132, 0.8)",
                  ],
